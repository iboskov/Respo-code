...
def importance_over_number(job_column_index, ocene_file, potrebno_file, table_of_importance, view, num=70,
    ...
    for i in range(len(ocene[:, 0])):
        ...
        if model == improve_comp_by_formula:
            if (potrebno[i, 0] >= num) and (model(0, ocene, potrebno, table_of_importance,
                                                  view) != "All competences satisfy the requirements."):
                enablePrint(printing)
                print("Over {} needs to be improved:".format(str(num)))
                return model(0, ocene, potrebno, table_of_importance, view)
        else:
            if (potrebno[i, 0] >= num) and (
                    model(0, ocene, potrebno, view) != "All competences satisfy the requirements."):
                enablePrint(printing)
                print("Over {} needs to be improved:".format(str(num)))
